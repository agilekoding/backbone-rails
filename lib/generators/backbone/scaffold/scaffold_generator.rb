    ...
    class ScaffoldGenerator < ModelGenerator
      ...
      def create_view_files
        available_views.each do |view|
          ...
          template "templates/#{view}.haml", File.join(backbone_path, "templates", plural_name, "#{view}.jst.ejs.haml")
        end

        template "views/model_view.coffee", File.join(backbone_path, "views", plural_name, "#{singular_name}_view.js.coffee")
        template "templates/model.haml", File.join(backbone_path, "templates", plural_name, "#{singular_name}.jst.ejs.haml")
      end
